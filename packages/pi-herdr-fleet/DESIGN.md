...
- **`worktree.remove` には明示的に長いタイムアウトを渡す。** トランスポートの既定は 5 秒だが、それは
  herdr の速さではなくクライアント側の待ち時間で、`node_modules` を含む checkout の削除はそれを超える。
  実測では herdr 側が削除を完了していたのにクライアントが先に諦め、続く `git branch -d` が
  「worktree がまだチェックアウトしている」で失敗し、後始末全体が失敗して 2 回呼ばないと通らなかった。
  30 秒（git の呼び出しと同じ）より長い 120 秒を渡す
- **「クライアントが諦めた」と「herdr が失敗した」は別。** タイムアウトは herdr の失敗ではないので、
  短く待って herdr 自身の `worktree.list` に記録の `path` がまだあるかを見る。無ければ成功として
  扱い（応答が遅かったことは警告として返す）、まだあれば失敗として扱う
- **途中で止まった後始末は記録に残す。** branch の削除が失敗したら `cleanedAt` は付けず、
  `cleanError` に理由とそこまで成功した段階（閉じた pane、worktree を消せたか）を書く。何も
  残さないと、pane だけ閉じて worktree と branch が残った run が「手つかず」に見える。記録を消さない
  原則はそのままで、次に最後まで通った clean が `cleanError` を消す
...
herdr のサイドバーが出せるのは pane の状態（idle / working / blocked / done）まで。Pi のセッション
JSONL には意味がある: どのモデルで、文脈が何トークンで、いくら使い、最後に何を言われ、いま何のツール
を実行しているか。**他の pane が何をしているかを、pane を切り替えずに分かる**ようにするのがこの節の
目的で、herdr が持つトポロジと生死に、Pi が持つ意味を繋ぐのが存在理由そのもの。

`/fleet view` で overlay を開く。引数なしの `/fleet` は承認ブローカーのまま。`ctrl+shift+a` も
ブローカーのまま。

### 一覧（1 pane = 1 行）

`pane_id`・自分印に続けて最後のユーザー発話、そのあとに agent 名と状態を ` · ` で繋ぐ。モデル・
文脈トークン・累計コストは一覧には出さず、詳細にだけ出す。行は pane の幅で切り詰める。agent 名の
無い pane は pane id を繰り返さない。

**最後のユーザー発話を pane id の直後（agent 名・状態より前）に置く。** これは実 pane で使って
決めた。80 桁の pane では `provider/model` とコストと文脈が幅を取り、意味がある唯一の列（最後の
ユーザー発話）が最初に消えた。「意味が見える」ための道具が狭い画面で意味を落とすのは本末転倒なので、
軸を pane id / 自分印 / 最後のユーザー発話 / agent 名 / 状態に絞り、数値は詳細へ移した。行は右から
切れるので、**先に置いたものだけが狭い pane に残る**。agent 名は branch 名であることが多く（fork の
既定）、例外ではなく普通に長い。後から読む人は、この順序を「情報が多い方が良い」で戻さないこと。

- 状態は herdr の `agent_status`。agent のいない pane は `unknown`
- セッションが無い pane は「Pi セッションなし」、パスはあるが読めない pane は「セッションを読めず」を
  **発話の位置**（agent 名・状態より前）に出す。行は右から切れるので、状態はそれより後ろで、狭い
  pane では先に消える
...
- 上限で切れたらコストが近似であると分かるようにする。コストは下限なので `≥` を付ける。文脈・最後の
  発話・実行中のツールは末尾の新しいエントリから取るので、tail の切り詰めでは失われない。ただし
  1 メッセージの文字数（2000）の天井で切られた発話はそこで終わる。これはメモリのための切り詰めで、
  `truncated` でも `≥` でもない（表示はもともと行幅で切れる）
- Pi のセッションが分からない pane（`agent_session` が無い、またはファイルがまだ無い）は、そう表示
  して落ちない。Pi は最初の assistant メッセージまでセッションファイルを書かないので、起動直後の
  pane はここに落ちる
- 更新は overlay を開いたときに 1 回取得し、`r` で再取得する。live 追従はこの増分ではやらない。
  イベント購読を増やして全セッションを読み直す価値が、その場で人間が開くビューには無い

### やらないこと

- 常時表示の widget / footer（ambient 版）。overlay だけ
- herdr サイドバーの再実装。herdr が持つ情報はそのまま herdr から読む
- 承認ブローカーの挙動変更
...
「テストの方針」は「入口は 2 つのまま」と書いている。これは Phase 1/2 を 30 秒で回せる速さを残す
ための規則で、その 2 本は `acceptance.sh` と `acceptance-fork.sh` を指す。フリートビューは 3 本目の
`acceptance-view.sh` にした。実 Pi セッションに実モデルで答えさせ、`working` の窓を捕まえる必要が
