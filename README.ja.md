...
| [pi-herdr-fleet](./packages/pi-herdr-fleet/) | [herdr](https://herdr.dev) の pane と Pi の間の承認ブローカー。`/fleet` が人間の返答を待っている pane を並べ、いま見ている pane から答えられる。tab のレイアウトの保存・復元と、`.env` / `.envrc` を引き継ぐ worktree 作成も担う。 |
