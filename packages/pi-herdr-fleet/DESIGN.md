...
- セッション末尾の読み取り、上限（バイト・行・文字）、`truncated` の伝播、近似の `≥`、tool call の
  抽出、agent のいない pane と読めないセッションの行、`self` の印と並び、一覧の列順（発話が agent 名・
  状態より前にあること）と pane id が繰り返されないこと → `selfcheck.ts`
